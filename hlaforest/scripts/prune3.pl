...
#        }
        ...
        foreach my $gene (keys %{$haplotypes_hash{ROOT}}) {
            $genes{$gene} = leaf_count($haplotypes_hash{ROOT}{$gene});
#            print $genes{$gene} . " haplotypes found for $gene\n";
        }
...
sub leaf_count {
    ...
    if (keys %{$hash_ptr}) {
        ...
        foreach my $key (keys %{$hash_ptr}) {
            $child_count += leaf_count($hash_ptr->{$key});
        }
        return $child_count;
    }
    else {
