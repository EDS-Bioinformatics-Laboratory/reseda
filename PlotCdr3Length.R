...
# Read CDR3 lengths
cdr3.length.all=read.csv("run06-bcrh-cdr3-length-all-but-reassigned.txt", header = FALSE)
cdr3.length.reassigned=read.csv("run06-bcrh-cdr3-length-reassigned.txt", header = FALSE)
...
plot(hist.cdr3.length.all, type="o", col="blue", xlab="CDR3 length", ylab="Frequency")
...
hist.trunc=hist.combined[1:28,] # TCR
hist.trunc=hist.combined[1:38,] # BCR
